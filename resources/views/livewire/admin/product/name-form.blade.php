    ...
    <div class="max-w-lg mx-auto mt-10 bg-white p-6">
        <form wire:submit.prevent="update">
            <div class="p-5 bg-gray-50 border border-gray-200">
                <label class="block text-[#7db0ad] text-sm font-semibold mb-2">Product Name</label>

                @if($isEditing)
                <div>
                    <input type="text" wire:model.live="name" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#7db0ad] hover:border-[#7db0ad] transition-all" placeholder="Enter new product name">
                ...
                <div class="flex items-center justify-between p-3 bg-gray-100 border border-gray-300 rounded-md hover:shadow-md hover:bg-gray-200 transition-all cursor-pointer">
                    <span class="text-[#7db0ad] font-medium">{{ $product->name}}</span>
